...
GET
  FILE=datamap + 'werkbestanden\eigendom_2022_basisafspraken.sav'.
DATASET NAME eigendommen WINDOW=FRONT.
...
GET
  FILE=datamap + 'werkbestanden\basisafspraken_alle_eigenaars_2022.sav'.
...
AGGREGATE
  /OUTFILE=* MODE=ADDVARIABLES
  /BREAK=eigendom_id
  /toegekende_huishoudens=sum(nn_toegekend).
...
compute #teverwijderen=0.
if huidig_bewoond<=toegekende_huishoudens & missing(nn_toegekend) #teverwijderen=1.
SELECT IF (#teverwijderen = 0).
EXECUTE.
...
if eigendom_id~=lag(eigendom_id) nn_toeken2=toegekende_huishoudens.
EXECUTE.
...
delete variables toegekende_huishoudens.
AGGREGATE
  /OUTFILE=* MODE=ADDVARIABLES
  /BREAK=eigendom_id
  /toegekende_huishoudens=sum(nn_toegekend).
...
if primaryfirst=1 & huidig_bewoond > toegekende_huishoudens nn_toegekend=1.
EXECUTE.
...
AGGREGATE
  ...
  /BREAK=eigendom_id
  /toegekende_huishoudens=sum(nn_toegekend).
recode toegekende_huishoudens (missing=0).
...
if indupgrp=1 & nationaal_nummer~=lag(nationaal_nummer) & huidig_bewoond>toegekende_huishoudens nn_toegekend=1.
if indupgrp=1 & MatchSequence=2 & missing(lag(nn_toegekend)) &  huidig_bewoond>toegekende_huishoudens nn_toegekend=1.
EXECUTE.
...
NATIONAAL_NUMMER
huidig_bewoond
zekerheid volgorde aandeel_eigendom.
EXECUTE.
recode volgorde (1 thru highest=1) (else=0).
RENAME VARIABLES volgorde=eigenaar.
